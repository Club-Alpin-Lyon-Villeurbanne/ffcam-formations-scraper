...
/** Certitude minimale par défaut pour retenir un mapping (surchargeable par appel) */
export const CERTAINTY_THRESHOLD = 70;

/**
 * Codes brevets → commission. Toutes les correspondances sont retenues (ex. BF1-RA-TR → randonnee et trail).
 *
...
const BREVET_PATTERNS: Array<{ pattern: RegExp; commission: string }> = [
  ...
  // AL-CG cascade de glace, AL-AL alpinisme, AL-GV grande voie
  { pattern: /^BF(\d|M)?-AL-/i, commission: 'alpinisme' },

  { pattern: /^BF(\d|M)?-CA-/i, commission: 'canyon' },

  // RA-RM rando montagne, RA-RAL rando alpine, RA-TR trail
  { pattern: /^BF(\d|M)?-RA-/i, commission: 'randonnee' },

  // SN-SR ski rando, SA ski alpin, RQ/RN raquette, SW/SWL snowboard rando, SWA snowboard alpin
...
/** SPORTS DE NEIGE, quand la discipline est donnée par les métadonnées */
...
/**
 * Discipline déduite de l'intitulé d'un niveau. Le premier pattern qui matche gagne :
 * ils sont rangés du plus spécifique au moins spécifique.
 *
 * Certitude :
...
/** Tableau vide pour un brevet transversal (ex. PSC1) */
...
/** Pour SPORTS DE NEIGE, dépend de la discipline : null sans discipline reconnue (voir getCommissionFromIntitule) */
...
/**
 * Commission d'un niveau de pratique, avec une certitude. Sert surtout pour SPORTS DE NEIGE,
 * où la discipline (ski de rando, snowboard, raquette…) se lit dans l'intitulé.
 *
 * @example
...
export function getCommissionFromIntitule(
  ...
  if (act && act !== 'SPORTS DE NEIGE') {
    ...
    // Activité non reconnue : on tente l'intitulé
  }

  // SPORTS DE NEIGE, activité absente ou non reconnue
...
/** Codes formations FFCAM → commission. Toutes les correspondances sont retenues. */
